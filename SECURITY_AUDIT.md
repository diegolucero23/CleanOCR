...
8. ✅ **Model revision pinning (#9)** — fixed 2026-07-08 (salvaged from the July 5 sprint branch). `LOCAL_GEMMA_REVISION` pins the HuggingFace revision of the local Gemma model (warns when unset); weight loading is safetensors-only so pickled `.bin` weights are never deserialized.
9. ✅ **`.env.redteam` template rename (#13)** — fixed 2026-07-08. `.env.redteam` (mock values only) replaced by tracked `.env.redteam.example`; the working copy is gitignored so real values can never be committed.
10. ✅ **Committed runtime artifacts (#14)** — the July 2026 history purge already stripped `logs.txt`, `failed_pages.log`, and the scan ZIP from the rewritten history; `.gitignore` now covers `logs.txt` and `*.zip` so they cannot return.

Still open: auth in front of `/uploads` and `/status` for any hosted deployment (#10 — deferred by owner decision 2026-07-07: deployment is single-user loopback-only; auth + CSRF + strict CORS become prerequisites if the bind ever moves off `127.0.0.1`).
